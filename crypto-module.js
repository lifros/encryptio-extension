/**
 * ENCRYPTIO - Crypto Module (ES6 Module version for background.js)
 * Wrapper che esporta le funzioni da crypto.js come ES6 module
 */

// Importa tutte le funzioni dal crypto.js originale (caricato come script globale)
// Nota: Questo file serve solo come wrapper per background.js che richiede ES6 modules
...
async function encryptTemporaryData(data) {
    try {
        ...
        const hmacKeyStr = await getHMACKey();
        const hmac = await generateHMAC(encryptedBase64, hmacKeyStr);
...
async function decryptTemporaryData(encryptedData) {
    try {
        ...
        if (hmac) {
            const hmacKeyStr = await getHMACKey();
            const isValid = await verifyHMAC(encryptedBase64, hmac, hmacKeyStr);
            if (!isValid) {
